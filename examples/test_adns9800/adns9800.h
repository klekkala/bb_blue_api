// a linux interface to the adns9800 laser mouse sensor
// using spidev driver and robotics cape library
// James Strawson 2014

// this is a derivative of joshuajnoble's arduino adns9800 example
// resources
...
int adns_write_reg(int fd, unsigned char reg_addr, unsigned char data){
...
// read motion registers one at a time
// this is mostly for development, use adns_read_burst instead
int adns_read_motion(int fd, int *dx, int *dy){
	unsigned char dx_l, dx_h, dy_l, dy_h;
	int16_t dx_16, dy_16;

	// must read motion register first to freeze delta registers
	adns_read_reg(fd, REG_Motion);
	// must read H after L or it clears the register
	dx_l = adns_read_reg(fd,REG_Delta_X_L);
	dx_h = adns_read_reg(fd,REG_Delta_X_H);
	dy_l = adns_read_reg(fd,REG_Delta_Y_L);
	dy_h = adns_read_reg(fd,REG_Delta_Y_H);

	// printf("xh: %.2x xl: %.2x ", dx_h, dx_l);
	// printf("yh: %.2x yl: %.2x " ,dy_h, dy_l);
	// printf("x: %s ", byte_to_binary(dx_h));
	// printf("%s ", byte_to_binary(dx_l));
	// printf("y: %s ",byte_to_binary(dy_h));
	// printf("%s ",byte_to_binary(dy_l));
	
	dx_16 = (int16_t) (dx_h<<8)|dx_l;
	dy_16 = (int16_t) (dy_h<<8)|dy_l;	
...
// burst reads motion and converts into integers
int adns_read_burst(int fd, int *dx, int *dy){
	int16_t dx_16, dy_16;
	// read burst mode
	struct spi_ioc_transfer xfer[1];
	unsigned char rd_buf[10];
	unsigned char wr_buf[1];
	memset(xfer, 0, sizeof xfer);
	memset(wr_buf, 0, sizeof wr_buf);
...
int adns_upload_firmware(int fd){
	struct spi_ioc_transfer xfer[1];
	unsigned char wr_buf[1];
	int i;

	printf("Uploading ADNS firmware\n");

	// send the firmware to the chip, cf p.18 of the datasheet
	// set the configuration_IV register in 3k firmware mode
	adns_write_reg(fd, REG_Configuration_IV, 0x02); // bit 1 = 1 for 3k mode


	// write 0x1d in SROM_enable reg for initializing
	adns_write_reg(fd, REG_SROM_Enable, 0x1d); 

	// wait for more than one frame period
	usleep(10000); // assume that the frame rate is as low as 100fps... even if it should never be that low
	...
	xfer[0].speed_hz = adns_speed;
    xfer[0].bits_per_word = adns_bits;
	ioctl(fd, SPI_IOC_MESSAGE(1), xfer); // write burst destination address
	usleep(15);

	// send all chars of the firmware
	for(i=0 ; i<firmware_length; i++){ 
		wr_buf[0] = firmware_data[i];
		xfer[0].tx_buf = (unsigned long) wr_buf; 
		xfer[0].len = 1;
		xfer[0].speed_hz = adns_speed;
		xfer[0].bits_per_word = adns_bits;
	...
	if(adns_read_reg(fd,REG_SROM_ID) != 164){
...
int initialize_adns9800(void){
	...
	ret = ioctl(adns_fd, SPI_IOC_WR_MODE, &adns_mode);
	if (ret == -1)
		printf("can't set spi mode");

	ret = ioctl(adns_fd, SPI_IOC_RD_MODE, &adns_mode);
	if (ret == -1)
		 printf("can't get spi mode");

	ret = ioctl(adns_fd, SPI_IOC_WR_BITS_PER_WORD, &adns_bits);
	if (ret == -1)
		 printf("can't set bits per word");

	ret = ioctl(adns_fd, SPI_IOC_RD_BITS_PER_WORD, &adns_bits);
	if (ret == -1)
		 printf("can't get bits per word");

	ret = ioctl(adns_fd, SPI_IOC_WR_MAX_SPEED_HZ, &adns_speed);
	if (ret == -1)
		 printf("can't set max speed hz");

	ret = ioctl(adns_fd, SPI_IOC_RD_MAX_SPEED_HZ, &adns_speed);
	if (ret == -1)
	...
	adns_read_reg(adns_fd,REG_Motion);
	adns_read_reg(adns_fd,REG_Delta_X_L);
	adns_read_reg(adns_fd,REG_Delta_X_H);
	adns_read_reg(adns_fd,REG_Delta_Y_L);
	adns_read_reg(adns_fd,REG_Delta_Y_H);
	
	// upload the firmware
	if(adns_upload_firmware(adns_fd)){
		printf("uploading to adns9800 failed, check connections\n");
		return -1;
	}
	usleep(5000);
	...
	wr_buf[0] = adns_read_reg(adns_fd, REG_LASER_CTRL0) & 0xf0;
	adns_write_reg(adns_fd, REG_LASER_CTRL0, wr_buf[0]);
	usleep(1000);

	printf("ADNS9800 Initialized\n");
	return adns_fd;
}
